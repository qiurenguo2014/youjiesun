...
#include "..\..\include\GUI_Private.h"
#include "..\..\include\LCD_SIM.h"
#include "..\..\include\LCD_ConfDefaults.h"
...
*/
extern void T6963_SetDot (int X_coor, int Y_coor, int color);
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
		...
		T6963_SetDot (xPhys,yPhys,PixelIndex);
		//printf("\r\nc:%u",PixelIndex);
      //
...
*   that no check on the parameters needs to be performed.
*/
extern unsigned int T6963_GetDot (int X_coor, int Y_coor);
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
		PixelIndex = T6963_GetDot(xPhys,yPhys);
      //
      // Write into hardware ... Adapt to your system
      //
      // TBD by customer...
...
static void * _GetDevData(GUI_DEVICE * pDevice, int Index) {
  GUI_USE_PARA(pDevice);
    ...
    case LCD_DEVDATA_MEMDEV:
      return (void *)&GUI_MEMDEV_DEVICE_32; // TBD: Has to be adapted to the right memory device depending on the used color depth!
    }
  #else
    GUI_USE_PARA(Index);
  #endif
